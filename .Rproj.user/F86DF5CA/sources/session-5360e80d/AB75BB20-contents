...
dge.dl_low <- dge.dl[, dge.dl$samples$group %in% c("Oil_low", "OPFR_low")]
dge.dl_low$samples$group <- droplevels(dge.dl_low$samples$group)
dge.dl_low$samples$group
dge.dl<- dge.dl_low
dge.dl$samples$group
...
coldata$group %>% 
  factor(.,levels = c("Oil_low","OPFR_low")) -> group.dl
...
contr.matrix <- makeContrasts(group.dlOil_low-group.dlOPFR_low,
                              levels = design.dl)
...
saveRDS(v.dl, "results/limma_vdl_Elevel_low.RDS")


### random sampling - don't redo unless needed ####
####### THIS WILL LIKELY NOT WORK SINCE ONLY HAVE FEW SAMPLES. -- attempting
#### auto-stopped at permutation 4208 - empty output
#### gonna rerun with 2500 permutes?
...
saveRDS(q.dl,("results/limma_vdl_cutoff5_2000_tworand_Elevel_low.RDS"))

#### done random sampling - don't redo unless needed ####


q.dl <- readRDS("results/limma_vdl_cutoff5_2000_tworand_Elevel_low.RDS")
...
Elevel_low_limma_results <- topTable(tmp1, sort.by = "P", n = Inf) %>% 
  rownames_to_column('symbol') %>% 
  dplyr::select(symbol,logFC,P.Value)

Elevel_low_limma_results1 <- Elevel_low_limma_results %>% 
...
saveRDS(Elevel_low_limma_results1,"results/Elevel_low_limma_results.RDS")
...
hist(Elevel_low_limma_results1$logFC)
