...
low <- readRDS("results/Elevel_low_limma_results.RDS")%>% filter(chr != "Y")
high <- readRDS("results/Elevel_high_limma_results.RDS")%>% filter(chr != "Y")


males_sigs <- males %>% 
  filter(logFC >= .33 | logFC <= -.33) %>%
  filter(P.Value < 0.05) %>% 
...
write.csv(conflict_table,"results/results_tables/conflict_table_033.csv", row.names = F)


# well, if we cutoff at 25%, there is 1 overlap (Atp2a3), and NO contrast. So, basically, drop this table entirely.
