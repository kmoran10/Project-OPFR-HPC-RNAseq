...
write.csv(MEsalmon.limma, "results/results_tables/TREATMENT_MEsalmon_limma.csv")
...
write.csv(MMsalmontop15, "results/results_tables/MMsalmon15.csv")
...
######## RETURN TO WHEN LOOKING AT ALL 6 PAIRWISES LATER!!! #########
...
# The pink module is higher in OPFR males

MEpink <- MEallcolors2 %>% 
  filter(MEcolor == "pink")

write.csv(MEpink, "results/MEpink.csv")


MMpink <- module.membership.measure2 %>% 
  filter(module == "MEpink") %>% 
  gather(., symbol, MM)
MMpink2 <- MMpink[-1,]
MMpink2 <- grcm38 %>% 
  select(symbol, chr, description) %>% 
  left_join(.,MMpink2, by = "symbol") %>% 
  filter(MM < 1)
...
write.csv(MMpink15, "results/results_tables/MMpink15.csv")


# The black module is lower in Oil high E females 

MEblack <- MEallcolors2 %>% 
  filter(MEcolor == "black")

write.csv(MEblack, "results/MEblack.csv")

MMblack <- module.membership.measure2 %>% 
  filter(module == "MEblack") %>% 
  gather(., symbol, MM)
MMblack2 <- MMblack[-1,]
MMblack2 <- grcm38 %>% 
  select(symbol, chr, description) %>% 
  left_join(.,MMblack2, by = "symbol") %>% 
  filter(MM < 1)
