...
dge.dl_high <- dge.dl[, dge.dl$samples$group %in% c("Oil_high", "OPFR_high")]
dge.dl_high$samples$group <- droplevels(dge.dl_high$samples$group)
dge.dl_high$samples$group
dge.dl<- dge.dl_high
dge.dl$samples$group
...
coldata$group %>% 
  factor(.,levels = c("Oil_high","OPFR_high")) -> group.dl
...
contr.matrix <- makeContrasts(group.dlOil_high-group.dlOPFR_high,
                              levels = design.dl)
...
saveRDS(v.dl, "results/limma_vdl_Elevel_high.RDS")
...
saveRDS(q.dl,("results/limma_vdl_cutoff5_2000_tworand_Elevel_high.RDS"))

#### done random sampling - don't redo unless needed ####


q.dl <- readRDS("results/limma_vdl_cutoff5_2000_tworand_Elevel_high.RDS")
...
Elevel_high_limma_results <- topTable(tmp1, sort.by = "P", n = Inf) %>% 
  rownames_to_column('symbol') %>% 
  dplyr::select(symbol,logFC,P.Value)

Elevel_high_limma_results1 <- Elevel_high_limma_results %>% 
...
saveRDS(Elevel_high_limma_results1,"results/Elevel_high_limma_results.RDS")
...
hist(Elevel_high_limma_results1$logFC)
