...
dge.dl_Oil <- dge.dl[, dge.dl$samples$group %in% c("Oil_M", "Oil_low", "Oil_high")]
dge.dl_Oil$samples$group <- droplevels(dge.dl_Oil$samples$group)
dge.dl_Oil$samples$group
dge.dl<- dge.dl_Oil
dge.dl$samples$group
...
coldata$group %>% 
  factor(.,levels = c("Oil_M","Oil_low","Oil_high")) -> group.dl
...
contr.matrix <- makeContrasts(group.dlOil_M-group.dlOil_low,
                              group.dlOil_M-group.dlOil_high,
                              group.dlOil_low-group.dlOil_high,
                              levels = design.dl)
...
saveRDS(v.dl, "results/limma_vdl_Oil.RDS")
...
saveRDS(q.dl,("results/limma_vdl_cutoff5_2000_tworand_Oil.RDS"))


#### done random sampling - don't redo unless needed ####


q.dl <- readRDS("results/limma_vdl_cutoff5_2000_tworand_Oil.RDS")
...
Oil_limma_results <- topTable(tmp1, sort.by = "P", n = Inf) %>% 
  rownames_to_column('symbol') %>% 
  dplyr::select(symbol,logFC,P.Value)

Oil_limma_results1 <- Oil_limma_results %>% 
...
saveRDS(Oil_limma_results1,"results/Oil_limma_results.RDS")


Oil_limma_results1 <- readRDS("results/Oil_limma_results.RDS")

### will need to adjust for 3 group Oil analysis
