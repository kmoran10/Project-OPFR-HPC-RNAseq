...
saveRDS(q.dl,("results/limma_vdl_cutoff5_2000_tworand_OPFR.RDS"))


#### done random sampling - don't redo unless needed ####


q.dl <- readRDS("results/limma_vdl_cutoff5_2000_tworand_OPFR.RDS")
...
OPFR_limma_results <- topTable(tmp1, sort.by = "P", n = Inf) %>% 
  rownames_to_column('symbol') %>% 
  dplyr::select(symbol,logFC,P.Value)

OPFR_limma_results1 <- OPFR_limma_results %>% 
...
saveRDS(OPFR_limma_results1,"results/OPFR_limma_results.RDS")


OPFR_limma_results1 <- readRDS("results/OPFR_limma_results.RDS")

### will need to adjust for 3 group OPFR analysis
...
hist(OPFR_limma_results1$logFC)
